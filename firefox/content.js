...
$(document).ready(() => {
	...
	$.get(browser.runtime.getURL('/content.html'), (data) => {
		...
		browser.runtime.sendMessage({request:"get-actions"}, (response) => {
			...
			if (window.location.href == browser.runtime.getURL("newtab.html")) {
				isOpen = true;
	...
	function renderAction(action, index, keys, img) {
		...
		if (!action.emoji) {
			...
			loadimg.onerror = () => {
				$(".omni-item[data-index='"+index+"'] img").attr("src", browser.runtime.getURL("/assets/globe.svg"));
			}
		}
	}
	...
	function closeOmni(restoreTab) {
		if (restoreTab !== false && window.location.href == browser.runtime.getURL("newtab.html")) {
	...
	function handleAction(e) {
		...
		if ($(".omni-extension input").val().toLowerCase().startsWith("/remove")) {
			...
			browser.runtime.sendMessage({request:action.action, tab:action, query:$(".omni-extension input").val()});
